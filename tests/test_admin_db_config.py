...
Locks in the alignment with turnstone-server:
  CLI / config.toml [database]  >  PEBBLE_DB_* env  >  hardcoded default
...
PEBBLE_DB_URL before every admin invocation.
"""
...
from pebble.admin import _get_storage
...
def _clear_db_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    """Clean slate: no PEBBLE_DB_* env vars unless a test sets them."""
    for var in (
        "PEBBLE_DB_BACKEND",
        "PEBBLE_DB_URL",
        "PEBBLE_DB_PATH",
...
def test_defaults_to_sqlite_when_neither_config_nor_env_set() -> None:
    ...
    with patch("pebble.core.storage.init_storage") as init:
        _get_storage(args)
...
def test_env_used_as_fallback_when_config_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PEBBLE_DB_BACKEND", "postgresql")
    monkeypatch.setenv("PEBBLE_DB_URL", "postgresql+psycopg://fromenv:x@host/db")
    monkeypatch.setenv("PEBBLE_DB_POOL_SIZE", "7")
    monkeypatch.setenv("PEBBLE_DB_SSLMODE", "require")
...
def test_config_toml_wins_over_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("PEBBLE_DB_BACKEND", "sqlite")
    monkeypatch.setenv("PEBBLE_DB_URL", "postgresql+psycopg://fromenv:x@host/db")
    monkeypatch.setenv("PEBBLE_DB_SSLMODE", "require")
...
def test_partial_config_falls_through_to_env_per_key(
    ...
    monkeypatch.setenv("PEBBLE_DB_SSLMODE", "require")
    monkeypatch.setenv("PEBBLE_DB_POOL_SIZE", "9")
...
def test_main_threads_config_toml_through_real_argv(
    ...
    fake_storage = patch("pebble.core.storage.init_storage").start()
    fake_storage.return_value.list_users.return_value = []
    try:
        from pebble.admin import main
...
def test_get_storage_initializes_real_sqlite_backend(tmp_path: Path) -> None:
    ...
    actually run.
    """
    from pebble.core.storage import reset_storage
