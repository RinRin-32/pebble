...
Usage:
    ...
    pebble-optimizer tests.json --no-optimize
    pebble-optimizer tests.json --prompt prompt.txt --n-runs 5 --max-iter 10
"""
...
from pebble.core.model_turn import cap_tool_calls, model_turn, resolve_lane
from pebble.core.providers import LLMProvider, create_provider
from pebble.core.session import ChatSession
from pebble.core.trajectory import Role, Turn
from pebble.eval.core import (
    _MCP_ONLY_TOOLS,
    BOLD,
    CYAN,
    DIM,
...
def run_optimization(
    ...
    if save_tools and best_node.tool_overrides:
        from pebble.core.tools import _TOOLS_DIR
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Examples:
              ...
              pebble-optimizer tests.json --prompt prompt.txt

              # Single evaluation pass (no optimization)
              pebble-optimizer tests.json --no-optimize

              # Configure runs and iterations
              pebble-optimizer tests.json --n-runs 5 --max-iter 10
        """),
    )
    parser.add_argument(
        "test_file",
