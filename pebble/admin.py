...
def _get_storage(args: argparse.Namespace) -> Any:
    ...
    Precedence (matches pebble-server): CLI / config.toml ``[database]``
    > ``PEBBLE_DB_*`` env vars > hardcoded defaults.
    """
    ...
    db_backend = str(_pick("db_backend", "PEBBLE_DB_BACKEND", "sqlite"))
    db_url = str(_pick("db_url", "PEBBLE_DB_URL"))
    db_path = str(_pick("db_path", "PEBBLE_DB_PATH"))
    db_pool_size = int(_pick("db_pool_size", "PEBBLE_DB_POOL_SIZE", "2"))
    return init_storage(
        db_backend,
        path=db_path,
        url=db_url,
        pool_size=db_pool_size,
        sslmode=str(_pick("db_sslmode", "PEBBLE_DB_SSLMODE")),
        sslrootcert=str(_pick("db_sslrootcert", "PEBBLE_DB_SSLROOTCERT")),
        sslcert=str(_pick("db_sslcert", "PEBBLE_DB_SSLCERT")),
        sslkey=str(_pick("db_sslkey", "PEBBLE_DB_SSLKEY")),
    )
...
def _cmd_create_admin(args: argparse.Namespace) -> None:
    ...
    from pebble.core.auth import hash_password, is_valid_username
...
def _cmd_tls_bootstrap(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print("lacme not installed. Run: pip install pebble-orchestrator[tls]", file=sys.stderr)
        sys.exit(1)
...
def _cmd_tls_list(args: argparse.Namespace) -> None:
    ...
    if jwt_secret:
        from pebble.core.auth import JWT_AUD_CONSOLE, ServiceTokenManager
...
def _discover_console_url() -> str:
    ...
    try:
        ...
        print(
            ...
            "directory with a pebble database.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _cmd_rerank_calibrate(args: argparse.Namespace) -> None:
    ...
    from pebble.core.rerank_calibrate import calibrate_model, merge_calibration_into_caps
...
def main() -> None:
    """Entry point for pebble-admin CLI."""
