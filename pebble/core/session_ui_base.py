...
Both :class:`pebble.server.WebUI` (interactive node UI) and
:class:`pebble.console.coordinator_ui.ConsoleCoordinatorUI` wrap a
:class:`~pebble.core.session.ChatSession` and fan events out over
...
:class:`pebble.core.session.SessionUI` by itself — ``on_state_change``
...
def _resolve_event_buffer_max() -> int:
    """Read ``PEBBLE_SSE_EVENT_BUFFER_MAX`` env override at import time.
...
def fire_judge_verdict_metric(
    ...
    Both :class:`pebble.server.WebUI` (per-node ``MetricsCollector``)
    and :class:`pebble.console.coordinator_ui.ConsoleCoordinatorUI`
...
class SessionUIBase:
    ...
    *concurrent* auxiliary thread (e.g. background title generation in
    ...
    def approve_tools(self, items: list[dict[str, Any]]) -> tuple[bool, str | None]:
        ...
        Shared body for both interactive (:class:`pebble.server.WebUI`)
        and coordinator (:class:`pebble.console.coordinator_ui.ConsoleCoordinatorUI`)
        ...
        if pending:
            try:
                from pebble.core.policy import evaluate_tool_policies_batch
                from pebble.core.storage._registry import get_storage
    ...
    def _tag_auto_approved(
        ...
        *,
        ...
        ``auto_approve``) so both :class:`pebble.server.WebUI` and
    ...
    def snapshot_and_consume_state_payload(self, state: str) -> dict[str, Any]:
        ...
        :meth:`pebble.console.coordinator_adapter.CoordinatorAdapter.emit_state`).
        """
