...
the operator to rediscover each symptom.  Run it inside a node::
    ...
    docker compose exec node-1 /app/.venv/bin/python -m pebble.core.preflight
...
def _check_agents() -> Check:
    from pebble.core.agents import ADAPTERS, available_agents
...
def _check_worktree_debt() -> Check:
    ...
    root = Path(os.environ.get("PEBBLE_WORKSPACE") or "/workspace") / "ws"
    if not root.is_dir():
