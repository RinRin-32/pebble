...
- Wall-clock deadline via :func:`pebble.core.deadline.run_abortable_with_deadline`,
...
from pebble.core.judge import (
    _CHARS_PER_TOKEN,
    _positive_window,
)
from pebble.core.log import get_logger
from pebble.core.model_turn import model_turn, resolve_capabilities, resolve_lane
...
if TYPE_CHECKING:
    ...
    from pebble.core.providers._protocol import LLMProvider, ModelCapabilities
...
class OutputGuardJudge:
    ...
    def _extract_client_config(client: Any, provider_name: str) -> dict[str, str]:
        ...
        the dict ``pebble.core.providers.create_client`` accepts.
    ...
    def evaluate(
        ...
        *,
        ...
        daemon worker via :func:`~pebble.core.deadline.run_abortable_with_deadline`
    ...
    def _user_prompt(
        ...
        *,
        ...
        :func:`pebble.core.fence.wrap`, which also neutralises any literal
