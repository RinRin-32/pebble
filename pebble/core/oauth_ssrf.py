...
Extracted from :mod:`pebble.core.oidc` so the per-(user, server) MCP
OAuth flow (see :mod:`pebble.core.mcp_oauth`) can reuse the exact same
...
async def validate_url_no_ssrf_async(
    ...
    :mod:`pebble.core.mcp_oauth`) wrap their validation calls in
