...
from pebble.core.skill_parser import ParsedSkill, parse_skill_md
    ...
    if resp.status_code == 403:
        ...
        if remaining == "0":
            raise SkillSourceError(
                ...
                "Set PEBBLE_GITHUB_TOKEN env var for higher limits (5000 req/hr)."
            )
    remaining = resp.headers.get("x-ratelimit-remaining", "")
...
async def fetch_skill_from_github(url: str) -> SkillPackage:
    ...
    Uses ``PEBBLE_GITHUB_TOKEN`` env var for authenticated requests
    ...
    token = os.environ.get("PEBBLE_GITHUB_TOKEN", "")
    if token:
