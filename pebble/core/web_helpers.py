...
def auth_user_id(request: Request) -> str:
    ...
    for handlers lifted into :mod:`pebble.core.session_routes`.
    """
...
def resolve_workstream_owner(
    ...
    *,
    ...
    gets a 403 (see
    :class:`pebble.core.auth.WorkstreamProjectVisibility`). Returns
    ...
    (:func:`pebble.core.memory.get_workstream_row` degrades an
    ...
    if owner is None:
        ...
        from pebble.core.memory import get_workstream_row
...
def parse_cors_origins() -> list[str] | None:
    """Parse ``PEBBLE_CORS_ORIGINS`` env var into a list of origin strings.

    Returns ``None`` when the variable is unset or empty (meaning: no CORS
    middleware, same-origin only).
    """
    cors_env = os.environ.get("PEBBLE_CORS_ORIGINS", "").strip()
    if not cors_env:
        return None
