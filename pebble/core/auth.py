...
class _DenyFilter:
    ...
    sentinel lives on :mod:`pebble.core.auth` so both modules share
...
def load_jwt_secret() -> str:
    ...
    """
    secret = os.environ.get("PEBBLE_JWT_SECRET", "").strip()
    if not secret:
    ...
    if not secret:
        log.error(
            "PEBBLE_JWT_SECRET is required but not set. "
...
class AuthMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if auth_result and auth_result.user_id:
            from pebble.core.log import ctx_user_id
...
async def handle_oidc_authorize(request: Request, audience: str) -> Response:
    ...
    # Build redirect URI (pinned by PEBBLE_OIDC_REDIRECT_BASE)
