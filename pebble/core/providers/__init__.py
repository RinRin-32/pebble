...
from pebble.core.providers._xai import XAI_DEFAULT_BASE_URL, XAIProvider
...
def create_client(provider_name: str, *, base_url: str, api_key: str) -> Any:
    ...
    if provider_name in ("anthropic", "anthropic-compatible"):
        from pebble.core.providers._anthropic import _ensure_anthropic
...
def list_known_models(provider: str) -> list[str]:
    ...
    if provider == "openai":
        from pebble.core.providers._openai_common import OPENAI_CAPABILITIES

        return sorted(OPENAI_CAPABILITIES.keys())
    if provider == "anthropic":
        from pebble.core.providers._anthropic import _ANTHROPIC_CAPABILITIES

        return sorted(_ANTHROPIC_CAPABILITIES.keys())
    if provider == "xai":
        from pebble.core.providers._xai import GROK_CAPABILITIES
