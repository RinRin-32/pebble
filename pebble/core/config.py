...
Config file resolution:
  ...
  2. ``$PEBBLE_CONFIG`` environment variable
...
#: Where this config lived when the project was called turnstone.  An existing
#: install must not lose its settings to a rename, so the old location is still
#: read when the new one has not been created yet.
LEGACY_CONFIG_DIR = Path("~/.config/turnstone").expanduser()


def _default_config_path() -> Path:
    """Prefer the current location, fall back to the pre-rename one."""
    ...
    if legacy.is_file():
        return legacy
    # Neither exists: name the current path, so anything that creates the file
...
def _resolve_config_path() -> Path:
    ...
    if _config_path is not None:
        return _config_path
    env = os.environ.get("PEBBLE_CONFIG", "").strip()
    if env:
...
def get_searxng_url() -> str | None:
    ...
    Precedence: config.toml [tools] searxng_url -> $PEBBLE_SEARXNG_URL.
    ...
    if cfg_url:
        ...
        return _searxng_url
    env_url = os.environ.get("PEBBLE_SEARXNG_URL", "").strip()
    if env_url:
...
def get_searxng_engines() -> str:
    ...
    Precedence: config.toml [tools] searxng_engines -> $PEBBLE_SEARXNG_ENGINES
    ...
    _searxng_engines = cfg or os.environ.get("PEBBLE_SEARXNG_ENGINES", "").strip()
...
def get_rerank_instruction() -> str:
    ...
    Precedence: config.toml [tools] rerank_instruction -> $PEBBLE_RERANK_INSTRUCTION
    ...
    if not _rerank_instruction_loaded:
        ...
        _rerank_instruction = cfg or os.environ.get("PEBBLE_RERANK_INSTRUCTION", "").strip()
...
def add_config_arg(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Path to config.toml (default: $PEBBLE_CONFIG or ~/.config/turnstone/config.toml)",
    )
