...
from pebble.core.config import get_searxng_engines, get_searxng_url
...
from pebble.core.mcp_client import try_prime_user_pools
...
from pebble.core.nudge_queue import (
    QUIET_CHANNEL,
    QUIET_DRAIN,
    TOOL_DRAIN,
    USER_DRAIN,
...
from pebble.core.providers import accumulate_tool_call_delta, create_provider
from pebble.core.ratelimit import TokenBucket
from pebble.core.safety import is_command_blocked, sanitize_command
from pebble.core.settings_registry import DEFAULT_AUTO_COMPACT_PCT
from pebble.core.skill_field_validation import SKILL_RUNTIME_CONFIG_FIELDS
from pebble.core.skill_parser import MAX_SKILL_DESCRIPTION_LEN
...
from pebble.core.tool_search import ToolSearchManager
...
from pebble.core.watch import WATCH_REMINDER_OPTIONAL_KEYS
from pebble.core.web import check_ssrf, fetch_with_ssrf_guard, strip_html
...
from pebble.ui.colors import DIM, GRAY, GREEN, RED, RESET, YELLOW, bold, cyan, dim
...
if TYPE_CHECKING:
    ...
    from pebble.core.judge import IntentJudge, JudgeConfig
    from pebble.core.mcp_client import MCPClientManager
    from pebble.core.model_registry import ModelRegistry
    from pebble.core.output_guard import OutputAssessment
    from pebble.core.output_guard_judge import OutputGuardJudge, OutputJudgeVerdict
    ...
    from pebble.core.rerank import RerankClient, Reranker
    from pebble.core.web_search import WebSearchClient
...
class _CancelRef(list[Any]):
    ...
    def aborted(self) -> bool:
        ...
        :class:`~pebble.core.deadline.StreamAbortRef` contract): a
...
# pebble.core.attachments so the server upload cap and the
...
def _prefix_sender_label(content: Any, sender: str, nonce: str) -> Any:
    ...
    (:func:`pebble.core.fence.wrap`) whose token lives only in the cached
    ...
    *content* is defanged first (:func:`~pebble.core.fence.neutralize` with
...
_SPEC_PLACEHOLDER_RE = re.compile(
    ...
    | \$\{(?P<env>(?:CLAUDE|TURNSTONE|PEBBLE)_[A-Z_]+)\}  # ${PEBBLE_*} + back-compat
    | \$(?P<idx_short>\d+)\b                          # $N
...
def _substitute_skill_args(
    ...
    * ``$<name>`` — named arg from the SKILL.md ``arguments:``
      ...
      index
    * ``${PEBBLE_SESSION_ID}`` / ``${CLAUDE_SESSION_ID}`` — current
    ...
    * ``${PEBBLE_EFFORT}`` / ``${CLAUDE_EFFORT}`` — reasoning_effort
    * ``${PEBBLE_SKILL_DIR}`` — absolute path of the materialized
    ...
    # ``PEBBLE_*`` is canonical; ``TURNSTONE_*`` and ``CLAUDE_*`` are
    # back-compat aliases carrying the same value, so skill files written
    # against either older spelling keep substituting after the rename.
    env = {
        "PEBBLE_SESSION_ID": ws_id,
        "TURNSTONE_SESSION_ID": ws_id,
        "CLAUDE_SESSION_ID": ws_id,
        "PEBBLE_EFFORT": effort,
        "TURNSTONE_EFFORT": effort,
        "CLAUDE_EFFORT": effort,
    }
    if skill_dir:
        # PEBBLE_SKILL_DIR only — NO CLAUDE_SKILL_DIR alias.  That name is
        # the host's in bash (see _skill_resource_env), so turnstone claims it
        # in neither surface to avoid a prompt-vs-bash divergence.
        env["PEBBLE_SKILL_DIR"] = skill_dir
...
class SessionUI(Protocol):
    ...
    def on_compaction(self, payload: dict[str, Any]) -> int | None:
        ...
        """
        from pebble.core.compaction_render import render_compaction_event_as_info
...
def _format_mcp_dispatch_error(prefix: str, exc: Exception) -> str:
    ...
    :func:`pebble.core.mcp_client._structured_error` payload (e.g.
...
def _notify_auth_headers() -> dict[str, str]:
    ...
    static_token = os.environ.get("PEBBLE_CHANNEL_AUTH_TOKEN", "").strip()
    if static_token:
        return {"Authorization": f"Bearer {static_token}"}

    # JWT via ServiceTokenManager
    jwt_secret = os.environ.get("PEBBLE_JWT_SECRET", "").strip()
    if not jwt_secret:
        return {}

    with _notify_token_lock:
        if _notify_token_manager is None:
            from pebble.core.auth import JWT_AUD_CHANNEL, ServiceTokenManager
...
class ChatSession:
    ...
    def __init__(
        ...
        if config_store is not None:
            try:
                from pebble.core.rule_registry import RuleRegistry
        ...
        if project_id and self._user_id:
            from pebble.core.auth import resolve_project_access
    ...
    def _resolve_search_client(self) -> WebSearchClient | None:
        ...
        ``storage → config.toml → env → registry default``:
          ...
          2. ``config.toml`` / the ``PEBBLE_SEARXNG_*`` env vars.
        ...
        """
        from pebble.core.web_search import resolve_web_search_client
    ...
    def _resolve_rerank_client(self) -> RerankClient | None:
        ...
        selected.
        """
        from pebble.core.rerank_config import resolve_rerank_client_from
    ...
    def _bm25_reranker(self, threshold: float = 0.0) -> Reranker | None:
        ...
        from pebble.core.rerank import RerankError, normalize_scores
    ...
    def _resolve_capabilities(
        ...
        Delegates to :func:`pebble.core.model_turn.resolve_capabilities` —
    ...
    def _finalize_provider_blocks(
        ...
        *,
        ...
        """Delegate to :func:`pebble.core.model_turn.finalize_provider_blocks`
    ...
    def _resolve_replay_reasoning_to_model(
        ...
        *,
        ...
        """Delegate to
        :func:`pebble.core.model_turn.resolve_replay_reasoning_to_model`.
    ...
    def _maybe_attach_vllm_chat_reasoning(
        ...
        """Delegate to
        :func:`pebble.core.model_turn.maybe_attach_vllm_chat_reasoning`
    ...
    def _render_skill_body(
        ...
        *,
        ...
        ``${PEBBLE_EFFORT}`` resolves identically wherever it runs.
    ...
    def _load_skills(self) -> None:
        ...
        ``${PEBBLE_SKILL_DIR}`` resolves to the concrete on-disk path
        ...
        # ``self._skill_resources_dir`` to resolve ``${PEBBLE_SKILL_DIR}``.
        ...
        if skill_data:
            ...
            if defaults:
                ...
                # subs (``${PEBBLE_SESSION_ID}`` / ``${PEBBLE_EFFORT}``)
    ...
    def _skill_resource_env(self) -> dict[str, str]:
        ...
        The bundle dir is exported as canonical ``PEBBLE_SKILL_DIR`` and
        ...
        env: dict[str, str] = {
            ...
            "PEBBLE_SKILL_DIR": self._skill_resources_dir,
        }
    ...
    def _persona_catalog_line(self) -> str:
        ...
        """
        from pebble.core.storage import get_storage, is_storage_initialized
    ...
    def set_watch_runner(self, runner: Any, wake_fn: Callable[[], object] | None = None) -> None:
        ...
        :func:`pebble.core.idle_nudge_watcher.wake_workstream_if_pending`
    ...
    def _pdf_rasterize_fallback_parts(
        ...
        from pebble.core.pdf import rasterize_pdf
    ...
    def _pdf_text_fallback_part(self, att: dict[str, Any]) -> dict[str, Any]:
        ...
        from pebble.core.pdf import extract_pdf_text
    ...
    def _stt_transcript_part(self, att: dict[str, Any]) -> dict[str, Any] | None:
        ...
        from pebble.core.audio import resolve_role_alias, transcribe_cached
    ...
    def _resolve_perception(
        ...
        from pebble.core.perception import PERCEPTION_SETTING
    ...
    def _perception_fallback_part(self, att: dict[str, Any], kind: str) -> dict[str, Any] | None:
        ...
        if kind == "audio" and not caps.supports_audio_input:
            return None
        from pebble.core.perception import describe_cached, describe_peek
    ...
    def _prepare_wire_messages(
        ...
        :func:`pebble.core.tool_advisory.make_system_turn`).  This pass
        folds them for the wire via
        :func:`pebble.core.lowering.fold_system_turns`: non-native
        ...
        *after* the fold so the fold-path wake turn, which the nudge folds into
        ...
        Before that, :func:`pebble.core.lowering.sanitize_tool_call_arguments`
        ...
        Finally, :func:`pebble.core.lowering.repair_wire_messages`
    ...
    def _provider_extra_params(
        ...
        """Delegate to :func:`pebble.core.model_turn.provider_extra_params`.
    ...
    def _append_system_turn(self, source: str, content: str, **meta: Any) -> None:
        ...
        :func:`pebble.core.tool_advisory.make_system_turn`) rather than
    ...
    def _compaction_event(self, my_generation: int, payload: dict[str, Any]) -> int | None:
        ...
        :meth:`SessionUIBase.on_compaction <pebble.core.session_ui_base.SessionUIBase.on_compaction>`
    ...
    def _ensure_judge(self) -> IntentJudge | None:
        ...
        if self._judge_config is None:
            return None
        try:
            from pebble.core.judge import IntentJudge
    ...
    def _ensure_output_guard_judge(self) -> OutputGuardJudge | None:
        ...
        if self._judge_config is None:
            return None
        try:
            from pebble.core.output_guard_judge import OutputGuardJudge
    ...
    def queue_message(
        ...
        """
        from pebble.core.tool_advisory import parse_priority
    ...
    def _flush_queued_messages(self, prefix: str = "") -> bool:
        ...
        """
        from pebble.core.tool_advisory import PRIORITY_IMPORTANT
    ...
    def _ensure_tool_call_ids(tool_calls: list[dict[str, Any]] | dict[int, dict[str, Any]]) -> None:
        ...
        Delegates to :func:`pebble.core.model_turn.ensure_tool_call_ids`
        ...
        if not self._resolve_search_client():
            return {
                ...
                "error": (
                    ...
                    "Set tools.searxng_url (or $PEBBLE_SEARXNG_URL) to a SearxNG "
    ...
    def _skill_hint(self, message: str, *, system_reminder: str = "") -> str:
        ...
        *message* is returned verbatim as the tool result.  It needs no escaping:
        ...
        (:func:`pebble.core.lowering._neutralize_host`) defangs it.
        """
        if system_reminder:
    ...
    def _prepare_skills_find(self, call_id: str, args: dict[str, Any]) -> dict[str, Any]:
        from pebble.core.skill_kind import SkillKind
    ...
    def _prepare_skills_create(self, call_id: str, args: dict[str, Any]) -> dict[str, Any]:
        from pebble.core.skill_field_validation import parse_skill_session_config
        from pebble.core.skill_kind import SkillKind
    ...
    # ``pebble.core.skill_field_validation`` so this and the admin
    ...
    def _scan_proposed_skill(self, content: str, allowed_tools: Any) -> str:
        ...
        fails to render.
        """
        from pebble.core.storage._utils import scan_skill_content
    ...
    def _exec_wait_for_workstream(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        # Inline import — ``pebble.core`` shouldn't import from
        # ``pebble.console`` at module load (layering), so the
        # tool-exec read pulls the canonical state set lazily.
        from pebble.console.coordinator_client import WAIT_REAL_TERMINAL_STATES
    ...
    def _workspace_cwd(self) -> str | None:
        ...
        :mod:`pebble.core.workspace`.
        """
        try:
            from pebble.core.workspace import resolve_cwd
    ...
    def _exec_bash(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        try:
            ...
            try:
                from pebble.core.env import scrubbed_env
    ...
    def _run_agent(
        ...
        while max_tool_turns < 0 or turn < max_tool_turns:
            ...
            # (:func:`pebble.core.model_turn.finalize_provider_blocks`).
    ...
    def _exec_task(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        if skill_data:
            ...
            # materialized yet), leaving ``${PEBBLE_SKILL_DIR}`` literal.
    ...
    def _exec_bind_repo(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        graph_note = ""
        try:
            from pebble.core.codegraph import index_worktree
    ...
    def _exec_setup_env(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        try:
            ...
            log.warning("setup_env.failed", action=action, exc_info=True)
            return _fail(f"Error: setup_env {action} failed: {exc}")

        out = self._truncate_output(out)
        self._report_tool_result(call_id, "setup_env", out)
        return call_id, out
    ...
    def _code_dispatch_denied(self) -> str:
        ...
        from pebble.core.access import can_dispatch_code
    ...
    def _exec_dispatch_agent(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        try:
            from pebble.core.codegraph import DEFAULT_MCP_SERVERS
    ...
    def _exec_watch(self, item: dict[str, Any]) -> tuple[str, str]:
        ...
        if action == "list":
            ...
            if not watches:
                ...
                self._report_tool_result(call_id, "watch", msg)
                return call_id, msg
            from pebble.core.watch import format_interval
