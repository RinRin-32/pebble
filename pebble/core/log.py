...
def configure_logging(
    ...
    *,
    ...
    level:
        ...
        ``CRITICAL``).  The ``PEBBLE_LOG_LEVEL`` env var, if set,
        overrides this.
    json_output:
        Force JSON (``True``) or console (``False``) output.  ``None``
        auto-detects: JSON when stderr is not a TTY.  The
        ``PEBBLE_LOG_FORMAT`` env var (``json`` / ``text``) overrides.
    service:
        Service name added to every log line (e.g. ``"server"``).
    """
    # Env-var overrides -------------------------------------------------------
    env_level = os.environ.get("PEBBLE_LOG_LEVEL", "").upper()
    if env_level:
        level = env_level

    env_fmt = os.environ.get("PEBBLE_LOG_FORMAT", "").lower()
