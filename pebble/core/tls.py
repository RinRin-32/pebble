...
def tls_pem_runtime_dir() -> Path:
    ...
    A fixed, well-known location (override: ``PEBBLE_TLS_PEM_DIR``) so the
    ...
    env = os.environ.get("PEBBLE_TLS_PEM_DIR")
...
def prepare_pem_runtime_dir() -> Path:
    ...
    private tmpfs; on bare metal set PEBBLE_TLS_PEM_DIR per node).
    """
    import os
    import shutil
    import stat
...
def _require_lacme() -> Any:
    try:
        ...
        raise ImportError(
            "lacme is required for TLS support. Install with: pip install pebble-orchestrator[tls]",
        ) from None
    return lacme
...
def build_cert_hostnames(
    ...
    *,
    ...
    # Reject wildcard / unspecified-address SANs so a stray PEBBLE_TLS_SANS
