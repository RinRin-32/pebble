...
:class:`~pebble.core.session_manager.SessionManager` (interactive
...
if TYPE_CHECKING:
    ...
    from pebble.core.attachments import UploadRejection
...
# :func:`pebble.core.memory.resolve_workstream` so callers can
...
# :func:`pebble.core.memory.get_workstream_display_names` so the
...
class SessionEndpointConfig:
    ...
    # :func:`pebble.core.memory.resolve_workstream` so user-friendly
...
# worker dispatch (driving :func:`pebble.core.session_worker.send`
...
def make_retry_handler(
    ...
    *,
    ...
    drives :func:`pebble.core.session_worker.send` with the kind's own
...
def make_open_handler(
    ...
    *,
    ...
    - ``cfg.open_resolve_alias`` — interactive wires
      :func:`pebble.core.memory.resolve_workstream` so callers
    ...
    async def create(request: Request) -> Response:
        ...
        try:
            if body_skill and not (isinstance(resume_ws_id_raw, str) and resume_ws_id_raw):
                ...
                # ``pebble.core.memory.get_skill_by_name`` — that
        ...
        if uploaded_files:
            ...
            if save_err is not None:
                from pebble.core.memory import delete_workstream as _delete_ws
...
async def _collect_saved_rows(
    ...
    from pebble.core.auth import WorkstreamProjectVisibility
    from pebble.core.memory import list_workstreams_with_history
...
def make_export_handler(cfg: SessionEndpointConfig) -> Handler:
    ...
    async def export(request: Request) -> Response:
        ...
        from pebble.core.export import WorkstreamNotFoundError, export_workstream
...
# (pebble.core.workstream, imported at module top): _PendingSend
...
def make_send_handler(cfg: SessionEndpointConfig) -> Handler:
    ...
    :func:`pebble.core.session_worker.send` (atomic spawn-or-enqueue under
    ...
    from pebble.core.web_helpers import auth_user_id, read_json_or_400
...
def make_attachment_handlers(cfg: SessionEndpointConfig) -> AttachmentHandlers:
    ...
    (:mod:`pebble.core.memory` calls are kind-agnostic) and the
    ...
    async def upload(request: Request) -> Response:
        from pebble.core.attachment_buffer import get_attachment_buffer
        from pebble.core.attachments import PDF_SIZE_CAP
    ...
    async def get_preview(request: Request) -> Response:
        ...
        from pebble.core.preview import PREVIEW_SERVE_MIMES, preview_response_headers
