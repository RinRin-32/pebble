...
if TYPE_CHECKING:
    ...
    from pebble.core.storage._notify import NotifyStream
...
#: application layer (via :mod:`pebble.core.mcp_crypto`, which re-exports
...
class StorageBackend(Protocol):
    ...
    **Tenancy filter on aggregates.**  Every list / count / aggregate
    ...
    ``_effective_user_filter`` in ``pebble.console.server``) and
    ...
    **Row access via ``_mapping``.**  List-style methods return
    ...
    def list_orphan_conversations(self) -> list[dict[str, Any]]:
        ...
        oldest-first.  Read-only; feeds the ``pebble-admin
