...
if TYPE_CHECKING:
    ...
    from pebble.core.storage._notify import Notify, NotifyStream
...
from pebble.core.storage._schema import (
    api_tokens,
    audit_events,
    channel_routes,
    channel_users,
...
from pebble.core.workstream import BULK_CLOSE_STATE_VALUES, WorkstreamKind
...
def _resolve_pg_listen_url(override: str, sqlalchemy_url: str) -> str:
    ...
    2. ``PEBBLE_DB_LISTEN_URL`` environment variable.
    3. The engine's main DB URL.

    The override is for deployments where the regular ``PEBBLE_DB_URL``
    ...
    override is set and ``PEBBLE_DB_URL`` already points at Postgres
    ...
    """
    raw = override.strip() or os.environ.get("PEBBLE_DB_LISTEN_URL", "").strip()
...
class PostgreSQLBackend:
    ...
    def listen(self, channels: Iterable[str]) -> Iterator[NotifyStream]:
        ...
        ``PEBBLE_DB_LISTEN_URL`` overrides the engine URL — see
    ...
    def get_node_metadata(self, node_id: str) -> list[dict[str, Any]]:
        from pebble.core.storage._schema import node_metadata
