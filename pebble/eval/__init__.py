"""pebble.eval — headless measurement substrate and its measure-only CLI.

The measurement substrate lives in :mod:`turnstone.eval.core`; the measure-only
``pebble-eval`` command lives in :mod:`turnstone.eval.cli`. The prompt
...
importers (e.g. ``from pebble.eval import score_run, _match_action``).
"""
