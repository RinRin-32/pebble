#!/usr/bin/env python3
"""cli.py — measure-only ``pebble-eval`` entry point.
...
Prompt optimization lives in :mod:`turnstone.optimizer` (``pebble-optimizer``).

Usage:
    pebble-eval tests.json
    pebble-eval tests.json --prompt prompt.txt --n-runs 5 --parallel 4
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            ...
            Prompt optimization now lives in pebble-optimizer.
        """),
    )
    parser.add_argument(
        "test_file",
