...
from pebble.core.session import ChatSession, SessionUI
...
def _handle_cluster_command(cmd_line: str, console_url: str | None) -> None:
    """Handle /cluster subcommands querying the pebble-console API."""
    import httpx
...
def detect_model(client: Any, provider: str = "openai") -> tuple[str, int | None]:
    """Auto-detect model — delegates to :func:`pebble.core.model_registry.detect_model`.

    CLI always uses fatal=True, so model is never None.
    """
    from pebble.core.model_registry import detect_model as _detect
...
def main() -> None:
    ...
    from pebble.core.log import configure_logging
    ...
    if mcp_client:
        ...
        from pebble.core.storage import get_storage as _cli_get_storage
