...
def build_shared_workstream_declaration(nonce: str) -> str:
    ...
    * **Authenticity** — only a ``[start sender-label_{nonce}]`` …
      ...
      attribution (:func:`pebble.core.fence.wrap` emits it; the wire copy
      also runs :func:`pebble.core.fence.neutralize` over participant content
...
def build_operator_instruction_declaration(nonce: str) -> str:
    ...
    marker so the fold (:func:`pebble.core.fence.wrap`) can rely on it: the
