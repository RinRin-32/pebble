...
the pebble server via HTTP.  An HTTP server runs alongside for inbound
notification delivery from the server.

Run as: ``pebble-channel --discord-token $PEBBLE_DISCORD_TOKEN``
"""
...
from pebble.core.log import add_log_args, configure_logging_from_args, get_logger

if TYPE_CHECKING:
    from collections.abc import Awaitable, Callable

    from pebble.channels._protocol import ChannelAdapter
    from pebble.core.storage import StorageBackend
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="turnstone channel gateway — bridges messaging platforms to the pebble cluster"
    )

    # -- Server connection ---------------------------------------------------
    parser.add_argument(
        "--server-url",
        default=os.environ.get("PEBBLE_SERVER_URL", ""),
        help="Turnstone server URL (default: $PEBBLE_SERVER_URL, or auto-discovered)",
    )
    parser.add_argument(
        "--console-url",
        default=os.environ.get("PEBBLE_CONSOLE_URL", ""),
        help="Console URL for multi-node routing (default: $PEBBLE_CONSOLE_URL). "
    ...
    parser.add_argument(
        "--discord-token",
        default=os.environ.get("PEBBLE_DISCORD_TOKEN", ""),
        help="Discord bot token (default: $PEBBLE_DISCORD_TOKEN)",
    )
    parser.add_argument(
        "--discord-guild",
        type=int,
    ...
    parser.add_argument(
        "--slack-token",
        default=os.environ.get("PEBBLE_SLACK_TOKEN", ""),
        help="Slack bot token (default: $PEBBLE_SLACK_TOKEN)",
    )
    parser.add_argument(
        "--slack-app-token",
        default=os.environ.get("PEBBLE_SLACK_APP_TOKEN", ""),
        help="Slack app-level token for Socket Mode (default: $PEBBLE_SLACK_APP_TOKEN)",
    )
    parser.add_argument(
        "--slack-channels",
        default=os.environ.get("PEBBLE_SLACK_CHANNELS", ""),
        help="Comma-separated list of allowed Slack channel IDs (default: all)",
    )
    parser.add_argument(
        "--slack-slash-command",
        default=os.environ.get("PEBBLE_SLACK_SLASH_COMMAND", "/turnstone"),
    ...
    parser.add_argument(
        "--http-port",
        type=int,
        default=int(os.environ.get("PEBBLE_CHANNEL_PORT", "8091")),
        help="HTTP server port (default: $PEBBLE_CHANNEL_PORT or 8091)",
    )
...
def _build_token_factories(
    ...
    from pebble.core.auth import JWT_AUD_CONSOLE, JWT_AUD_SERVER, ServiceTokenManager
...
def _resolve_advertise_url(args: argparse.Namespace) -> str:
    ...
    override = os.environ.get("PEBBLE_CHANNEL_ADVERTISE_URL", "").strip()
    if override:
        return override
...
async def _run_gateway(
    ...
    from pebble.channels._http import _get_service_id
...
def main() -> None:
    ...
    from pebble.core.storage._registry import get_storage, init_storage
    ...
    init_storage(
        backend=os.environ.get("PEBBLE_DB_BACKEND", "sqlite"),
        url=os.environ.get("PEBBLE_DB_URL", ""),
        path=os.environ.get("PEBBLE_DB_PATH", ""),
    )
    storage = get_storage()
    ...
    if has_adapters:
        ...
        log.warning(
            ...
            hint="set $PEBBLE_DISCORD_TOKEN or $PEBBLE_SLACK_TOKEN and restart",
        )
        adapters = {}
