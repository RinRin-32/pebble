...
from pebble.console.coordinator_alias import resolve_coordinator_alias
from pebble.console.coordinator_client import load_task_envelope
from pebble.console.metrics import ConsoleMetrics
from pebble.console.router import ConsoleRouter
...
from pebble.core.web_helpers import (
    read_json_or_400,
...
log = logging.getLogger("pebble.console.server")
...
async def cluster_ws_detail(request: Request) -> JSONResponse:
    ...
    """
    from pebble.core.auth import WorkstreamProjectVisibility, require_permission
...
async def list_available_models(request: Request) -> JSONResponse:
    ...
    from pebble.core.providers.effort_ladder import effort_ladder_for_model
    ...
    # :func:`pebble.console.coordinator_alias.resolve_coordinator_alias`).
...
async def create_workstream(request: Request) -> JSONResponse:
    ...
    from pebble.core.auth import require_any_permission
    from pebble.core.web_helpers import read_json_or_400, read_multipart_create_or_400
...
# ``patch("pebble.console.server.auth_login")`` is observed; a dict
...
# them into pebble.core.session_routes with kind branching.
# ---------------------------------------------------------------------------
...
def _coord_dispatch_retry(ws: Workstream, user_msg: str) -> None:
    ...
    :func:`pebble.core.session_worker.send` dispatcher — but without
...
def _coord_events_replay(
    ...
    1. ``connected`` + optional ``status`` via the shared
       :func:`pebble.core.session_replay.session_replay_preamble`
...
async def _coord_create_post_install(
    ...
    from pebble.core.attachments import resolve_staged_attachments
...
def _bootstrap_coord_subsystem(
    ...
    from pebble.console.coordinator_idle_observer import (
...
async def admin_create_user(request: Request) -> JSONResponse:
    ...
    from pebble.core.auth import hash_password, require_permission
    from pebble.core.web_helpers import require_storage_or_503
    ...
    from pebble.core.auth import is_valid_username
...
async def admin_create_token(request: Request) -> JSONResponse:
    ...
    from pebble.core.auth import generate_token, hash_token, require_permission, token_prefix
    from pebble.core.web_helpers import require_storage_or_503
...
async def admin_delete_oidc_identity(request: Request) -> JSONResponse:
    ...
    if token_store is not None:
        ...
        try:
            from pebble.core.mcp_oauth import obo_server_names
...
async def admin_create_schedule(request: Request) -> JSONResponse:
    ...
    from pebble.server import _validate_notify_targets
...
async def admin_create_role(request: Request) -> JSONResponse:
    ...
    from pebble.core.audit import record_audit
    from pebble.core.auth import is_valid_username, require_permission
    from pebble.core.web_helpers import read_json_or_400, require_storage_or_503
...
def _parse_skill_session_config(body: dict[str, Any]) -> tuple[dict[str, Any], JSONResponse | None]:
    ...
    Validation lives in ``pebble.core.skill_field_validation`` so the
...
async def list_skills_summary(request: Request) -> JSONResponse:
    ...
    from pebble.core.web_helpers import require_storage_or_503, skill_summary_rows
...
def _get_discovery_url(request: Request) -> str:
    ...
    from pebble.core.skill_sources import DEFAULT_DISCOVERY_URL
...
async def admin_parse_skill(request: Request) -> JSONResponse:
    ...
    """
    from pebble.core.auth import require_permission
    from pebble.core.skill_parser import parse_skill_md
    from pebble.core.web_helpers import read_json_or_400
...
async def admin_skill_discover(request: Request) -> JSONResponse:
    ...
    from pebble.core.auth import require_permission
    from pebble.core.skill_sources import SkillSourceError, SkillsShClient
    from pebble.core.web_helpers import require_storage_or_503
...
async def admin_list_settings(request: Request) -> JSONResponse:
    ...
    from pebble.core.auth import require_permission
    from pebble.core.settings_registry import SETTINGS, deserialize_value
    from pebble.core.web_helpers import require_storage_or_503
...
async def admin_delete_setting(request: Request) -> JSONResponse:
    ...
    from pebble.core.audit import record_audit
    from pebble.core.auth import require_permission
    from pebble.core.settings_registry import validate_key
    from pebble.core.web_helpers import require_storage_or_503
...
def _get_registry_url(request: Request) -> str:
    ...
    from pebble.core.config import load_config
    from pebble.core.mcp_registry import DEFAULT_REGISTRY_URL
...
def _enforce_oauth_user_https(auth_type: str, url: str | None) -> JSONResponse | None:
    ...
    with :func:`pebble.core.mcp_client._validate_oauth_user_url`
...
def _enforce_oauth_obo_requirements(
    ...
    *,
    ...
    if check_oidc_deployment:
        ...
        from pebble.core.mcp_oauth import OBO_GRANT_PROFILES
...
async def admin_mcp_bulk_revoke(request: Request) -> JSONResponse:
    ...
    See :func:`pebble.core.mcp_client._dispatch_pool` retry path.
    """
    from pebble.core.audit import record_audit
    from pebble.core.auth import require_permission
    from pebble.core.web_helpers import require_storage_or_503
...
# Keep in sync with pebble.core.providers._VALID_API_SURFACES.
...
# Keep in sync with pebble.core.providers._google.GOOGLE_DEFAULT_BASE_URL
...
async def admin_list_model_definitions(request: Request) -> JSONResponse:
    ...
    if not configured_default:
        from pebble.core.config import load_config as _load_cfg
...
def _global_rerank_instruction(app_state: Any) -> str:
    ...
    """
    from pebble.core.config import get_rerank_instruction
...
def _run_rerank_calibration(
    ...
    """
    from pebble.core.rerank_calibrate import calibrate_model
...
async def admin_detect_model(request: Request) -> JSONResponse:
    ...
    if bool(body.get("supports_rerank")):
        ...
        from pebble.core.rerank_calibrate import calibration_caps_fields
...
async def admin_calibrate_model_definition(request: Request) -> JSONResponse:
    ...
    """
    from pebble.core.auth import require_permission
    from pebble.core.rerank_calibrate import merge_calibration_into_caps
    from pebble.core.web_helpers import require_storage_or_503
...
async def admin_list_personas(request: Request) -> JSONResponse:
    ...
    """
    from pebble.core.auth import require_permission
    from pebble.core.tools import COORDINATOR_TOOLS, INTERACTIVE_TOOLS
    from pebble.core.web_helpers import require_storage_or_503
...
async def admin_list_heuristic_rules(request: Request) -> JSONResponse:
    ...
    # Get built-in rules
    from pebble.core.judge import _HEURISTIC_RULES
...
async def admin_list_output_guard_patterns(request: Request) -> JSONResponse:
    ...
    from pebble.core.output_guard import _BUILTIN_OG_PATTERNS
...
async def tls_ca_status(request: Request) -> JSONResponse:
    ...
    from pebble.console.tls import _CA_CN
...
def create_app(
    *,
    ...
    from pebble.core.attachments import classify_upload as _coord_classify_upload
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Examples:
              pebble-console                              # default settings
              pebble-console --port 9090                  # custom port
        """),
    )
    parser.add_argument(
        "--host",
    ...
    try:
        ...
        db_backend = os.environ.get("PEBBLE_DB_BACKEND", "sqlite")
        db_url = os.environ.get("PEBBLE_DB_URL", "")
        db_path = os.environ.get("PEBBLE_DB_PATH", "")
        ...
        db_listen_url = getattr(args, "db_listen_url", None) or os.environ.get(
            "PEBBLE_DB_LISTEN_URL", ""
        )
        auth_storage = init_storage(
            db_backend,
            path=db_path,
            url=db_url,
            sslmode=os.environ.get("PEBBLE_DB_SSLMODE", ""),
            sslrootcert=os.environ.get("PEBBLE_DB_SSLROOTCERT", ""),
            sslcert=os.environ.get("PEBBLE_DB_SSLCERT", ""),
            sslkey=os.environ.get("PEBBLE_DB_SSLKEY", ""),
    ...
    if auth_storage is None:
        log.error(
            ...
            "Set PEBBLE_DB_PATH or PEBBLE_DB_URL."
        )
        raise SystemExit(1)
    ...
    # Precedence: PEBBLE_CONSOLE_URL env > auto-detect from bind address.
    # In Docker Compose, set PEBBLE_CONSOLE_URL to the service name (e.g. http://console:8090).
    import socket as _socket

    _console_url_env = os.environ.get("PEBBLE_CONSOLE_URL", "")
    if _console_url_env:
