...
// the SettingDef lands in pebble/core/settings_registry.py.
...
// session model`` per pebble/core/settings_registry.py — there's
...
// pebble/core/audio.py model_supports_role: an explicit capability flag
...
// pebble/core/audio.py (the canonical source). A substring match marks
...
// _AUDIO_SDK_PROVIDERS / _provider_carries_audio in pebble/core/audio.py:
...
function _audioModelEligible(md, capFlag, mediaRole) {
  ...
  // path — mirror model_supports_role in pebble/core/audio.py.
...
function loadCodingJobs() {
  ...
  authFetch("/v1/api/admin/coding-jobs")
    ...
    .then(function (data) {
      ...
      if (!jobs.length) {
        setSafeHtml(host, '<div class="dashboard-empty">No workstream is bound to a repository yet. Use bind_repo, or: pebble-client dispatch --repo &lt;name&gt; "task"</div>');
        return;
      }
