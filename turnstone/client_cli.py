"""``turnstone-client`` — drive a remote turnstone from your own machine.
...
Credentials live in ``~/.config/turnstone/client.toml`` (mode 0600), matching
the config location the rest of turnstone already uses.  ``TURNSTONE_URL`` and
``TURNSTONE_TOKEN`` override the file, so CI can pass them in the environment
...
def load_config(path: Path = CONFIG_PATH) -> ClientConfig:
    ...
    cfg.url = os.environ.get("TURNSTONE_URL") or cfg.url
    cfg.token = os.environ.get("TURNSTONE_TOKEN") or cfg.token
    return cfg
...
def _require(cfg: ClientConfig) -> None:
    if not cfg.configured:
        raise SystemExit(
            ...
            "  turnstone-client login --url https://host:8443 --token ts_...\n"
            "or set TURNSTONE_URL and TURNSTONE_TOKEN."
        )
...
def cmd_login(args: argparse.Namespace) -> int:
    ...
    if not cfg.token:
        raise SystemExit("--token is required (create one with: turnstone-admin create-token)")
...
def cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    if not args.json:
        ...
        print(f"  follow:  turnstone-client ws list")
        print(f"  approve: turnstone-client approve {ws_id[:12]}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="turnstone-client",
        description="Drive a remote turnstone cluster from your own machine.",
    )
    ...
    p_login.add_argument("--token", default="", help="API token (turnstone-admin create-token)")
