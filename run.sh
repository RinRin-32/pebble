...
#
# Env overrides:
#   PEBBLE_DIR   where to clone (default: $HOME/turnstone)
#   PEBBLE_REPO  git URL (default: https://github.com/RinRin-32/pebble.git)

set -Eeuo pipefail

REPO_URL="${PEBBLE_REPO:-https://github.com/RinRin-32/pebble.git}"
INSTALL_DIR="${PEBBLE_DIR:-$HOME/turnstone}"
...
clone_repo() {
    if [ -d "$INSTALL_DIR/.git" ]; then
        ...
        [ -e "$INSTALL_DIR" ] && [ -n "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ] \
            && die "$INSTALL_DIR exists and is not a turnstone checkout. Set PEBBLE_DIR to an empty path."
...
${GREEN}${BOLD}Turnstone is running${RESET} (${NODE_COUNT} node$([ "$NODE_COUNT" = 1 ] || echo s)).
  ...
  Finish setup
    ...
    cd $INSTALL_DIR && $DOCKER compose exec node-1 pebble-admin create-admin --username admin --name "Admin"${RESET}
  ...
  Troubleshoot  ${DIM}pipx run --spec turnstone pebble-doctor --dir $INSTALL_DIR${RESET}
